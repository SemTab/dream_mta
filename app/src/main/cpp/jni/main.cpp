...
// Add extern declaration for javaVM at the top level
...
void InitSAMP(JNIEnv* pEnv, jobject thiz)
{
	...
	g_pszStorage = "/storage/emulated/0/Dream/";
	if(!g_pszStorage)
	{
...
void InitInMenu()
{
	...
	pGUI = new CGUI();
	pKeyBoard = new CKeyBoard(); // Этот класс теперь реализован как пустышка
	...
	// Безопасный вызов функции клавиатуры
	try {
		if (pKeyBoard && pSettings) {
			ProcessCheckForKeyboard();
		}
	} catch (...) {
		Log("Error in ProcessCheckForKeyboard");
	}
...
void ProcessCheckForKeyboard()
{
	// Отключено для предотвращения сбоев
	if (pKeyBoard && pSettings)
	{
...
void exitCase()
{
    // Get a fresh JNIEnv pointer instead of calling through pJava->GetEnv()
    JNIEnv* env = nullptr;
    if (javaVM->GetEnv((void**)&env, JNI_VERSION_1_4) != JNI_OK || !env) {
        Log(weikton(">> Failed to get JNIEnv, exiting..."));
        std::terminate();
        return;
    }

    // Check for existing exceptions before making JNI calls
    ...
    try {
        jobject activity = GetGlobalActivity(env);
        if (!activity) {
            Log(weikton(">> Failed to get activity, exiting..."));
            std::terminate();
            return;
        }
        
        jPackageName = GetPackageName(env, activity);
        if (!jPackageName) {
            Log(weikton(">> Failed to get package name, exiting..."));
            std::terminate();
            return;
        }
        ...
        if (!packageName) {
            Log(weikton(">> Failed to get package name chars, exiting..."));
            std::terminate();
            return;
        }
        
        bool result = isWNHere(packageName);
        ...
        Log(weikton(">> Exception in exitCase, exiting..."));
        std::terminate();
    }
}
...
void InitInGame()
{
	...
	if (!bNetworkInited)
	{
        ...
        // Создаем сетевой интерфейс без параметров
        pNetGame = new CNetGame("", 0, pSettings->GetReadOnly().szNickName, pSettings->GetReadOnly().szPassword);
        
        // Сразу показываем окно выбора серверов
        if (pNetGame) {
            pNetGame->ShowServerSelector();
        }
...
void CTimer__StartUserPause_hook()
{
	if (pJava)
	{
		// Безопасный вызов функций клавиатуры
		try {
			if (pKeyBoard)
			{
			...
			Log("Error in keyboard handling during pause");
		}
