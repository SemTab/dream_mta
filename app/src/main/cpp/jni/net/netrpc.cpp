...
void InitGame(RPCParameters *rpcParams)
{
	...
	// Проверяем, что размер данных адекватный
	if (iBitLength <= 0 || iBitLength > 65535 * 8) {
		Log("InitGame: Некорректный размер данных: %d бит", iBitLength);
		return;
	}
	...
	if (pNetGame->m_iSpawnsAvailable < 0 || pNetGame->m_iSpawnsAvailable > 5000) {
		Log("Count = %d", pNetGame->m_iSpawnsAvailable);
		pNetGame->m_iSpawnsAvailable = 0;
	}
	...
	if(byteStrLen > 0 && byteStrLen < sizeof(pNetGame->m_szHostName))
	{
	...
	{
		if (byteStrLen > 0)
			Log("InitGame: Некорректная длина имени хоста: %d", byteStrLen);
		strcpy(pNetGame->m_szHostName, "Unknown");
	}

	// Проверка чтения моделей транспорта
	uint8_t byteVehicleModels[212];
	if (bsInitGame.GetNumberOfUnreadBits() >= 212 * 8) {
		bsInitGame.Read((char*)&byteVehicleModels[0], 212);							// don't use?
	} else {
		Log("InitGame: Недостаточно данных для чтения моделей транспорта");
		memset(byteVehicleModels, 0, sizeof(byteVehicleModels));
	}
	
	// Чтение friendly fire только если есть данные
	if (bsInitGame.GetNumberOfUnreadBits() >= 32) {
		bsInitGame.Read(pNetGame->m_iVehicleFriendlyFire);							// native EnableVehicleFriendlyFire(); +
	} else {
		pNetGame->m_iVehicleFriendlyFire = 0;
	}
...
void ClientMessage(RPCParameters *rpcParams)
{
	...
	// Защита от переполнения памяти - проверяем, что длина строки разумная
	if(dwStrLen > 4096 || dwStrLen == 0) {
		Log("ClientMessage: Слишком большая длина строки (%u), игнорируем сообщение", dwStrLen);
		return;
	}
	
	char* szMsg = (char*)malloc(dwStrLen+1);
	if(!szMsg) {
		Log("ClientMessage: Не удалось выделить память для сообщения");
		return;
	}
...
void ConnectionRejected(RPCParameters *rpcParams)
{
	...
	else if(byteRejectReason == REJECT_REASON_BAD_NICKNAME)
	{
		...
		pChatWindow->AddInfoMessage("Выйдите из игры, зайдите в лаунчер Dream Mobile и измените ник!");
	}
