...
bool ProcessVoiceCommands(const char* str)
{
	...
	if (strstr(str, "/debug"))
	{
		...
		pChatWindow->AddDebugMessage("{FFFFFF}| debug toggle by BD");
		return true;
	}

	//

//	// Добавляем обработку команды /servers для вызова окна выбора серверов
//	if (strstr(str, "/servers"))
//	{
//		pJava->ShowIngameServerSelector();
//		return true;
//	}
	...
	if (strstr(str, "/pvoice"))
	{
		...
		if (g_bShowVoiceList)
		{
			pChatWindow->AddDebugMessage("{FFFF00}[VOICE]{FFFFFF}: ����������� ��������� ��������");
		}
		else
		{
			pChatWindow->AddDebugMessage("{FFFF00}[VOICE]{FFFFFF}: ����������� ��������� ���������");
	...
	if (strstr(str, "/vmute"))
	{
		...
		{
			pChatWindow->AddDebugMessage("{FFFF00}[VOICE]{FFFFFF}: ����������� /vmute [ID ������]");
			return true;
		}
		if (id < 0 || id > 1000)
		{
			pChatWindow->AddDebugMessage("{FFFF00}[VOICE]{FFFFFF}: ID �� ������ ���� ����� {ffff00}0{ffffff} � ����� {ffff00}1000");
			return true;
		}
		pVoice->MutePlayer(id);
		pChatWindow->AddDebugMessage("{FFFF00}[VOICE]{FFFFFF}: {FFFF00}ID %d{FFFFFF} ��� ��������", id);
		return true;
	}
	...
	if (strstr(str, "/vunmute"))
	{
		...
		if (sscanf(str, "%d", &id) == -1)
		{
			pChatWindow->AddDebugMessage("{FFFF00}[VOICE]{FFFFFF}: ����������� /vunmute [ID ������]");
			return true;
		}
		if (id < 0 || id > 1000)
		{
	...
	if (strstr(str, "/voicevolume"))
	{
		...
		{
			pChatWindow->AddDebugMessage("{FFFF00}[VOICE]{FFFFFF}: ����������� /voicevolume [������� ��������� �� 0 �� 100]");
			return true;
		}
		pChatWindow->AddDebugMessage("{FFFF00}[VOICE]{FFFFFF}: ����������� ����� ��������� ����-���� ({ffff00}%d{ffffff})", volume);
	...
	if (strstr(str, "/vplayer"))
	{
		...
		{
			pChatWindow->AddDebugMessage("{FFFF00}[VOICE]{FFFFFF}: ����������� /vplayer [ID ������] [������� ��������� �� 0 �� 100]");
			return true;
		}
		if (id < 0 || id > 1000)
		{
		...
		pChatWindow->AddDebugMessage("{FFFF00}[VOICE]{FFFFFF}: ��� {ffff00}ID %d{ffffff} ��� ���������� ������� ��������� {ffff00}%d", volume, id);
		return true;
	}
	return false;
...
{
	// Отключаем рендеринг дефолтного чата
	return;
...
void CChatWindow::AddChatMessage(char* szNick, uint32_t dwNickColor, char* szMessage)
{
	...
	// Отправка сообщения в Java-чат
	char fullMessage[MAX_MESSAGE_LENGTH * 3 + 1];
	if (strlen(szNick) > 0) {
		sprintf(fullMessage, "{%06X}%s: {%06X}%s", dwNickColor >> 8, szNick, m_dwTextColor >> 8, szMessage);
	} else {
		sprintf(fullMessage, "{%06X}%s", m_dwTextColor >> 8, szMessage);
	}
	
	// Вызываем Java-метод для добавления сообщения в чат
	if (pJava) {
		pJava->AddChatMessage(fullMessage);
	}
}

void CChatWindow::AddInfoMessage(char* szFormat, ...)
{
	char szMessage[MAX_MESSAGE_LENGTH * 3 + 1];
	memset(szMessage, 0, MAX_MESSAGE_LENGTH * 3 + 1);

	va_list args;
	va_start(args, szFormat);
	vsnprintf(szMessage, MAX_MESSAGE_LENGTH * 3, szFormat, args);
	va_end(args);

	AddToChatWindowBuffer(CHAT_TYPE_INFO, szMessage, nullptr, m_dwInfoColor, 0);
	
	/* 
	// Отправка информационного сообщения в Java-чат
	char fullMessage[MAX_MESSAGE_LENGTH * 3 + 1];
	sprintf(fullMessage, "{%06X}* %s", m_dwInfoColor >> 8, szMessage);
...
void CChatWindow::AddDebugMessage(char* szFormat, ...)
{
	...
	AddToChatWindowBuffer(CHAT_TYPE_DEBUG, szMessage, nullptr, m_dwDebugColor, 0);
	
	// Отправка отладочного сообщения в Java-чат
	char fullMessage[MAX_MESSAGE_LENGTH * 3 + 1];
	sprintf(fullMessage, "{%06X}# %s", m_dwDebugColor >> 8, szMessage);
...
void CChatWindow::AddDebugMessageNonFormatted(char* szStr)
{
	...
	// Отправка отладочного сообщения в Java-чат
	char fullMessage[MAX_MESSAGE_LENGTH * 3 + 1];
	sprintf(fullMessage, "{%06X}# %s", m_dwDebugColor >> 8, szStr);
...
void CChatWindow::AddClientMessage(uint32_t dwColor, char* szStr)
{
	...
	// Отправка клиентского сообщения в Java-чат
	char fullMessage[MAX_MESSAGE_LENGTH * 3 + 1];
	sprintf(fullMessage, "{%06X}%s", dwColor >> 8, szStr);
	...
	uint32_t dwTextColor, uint32_t dwNickColor)
{
	...
	// Отправляем сообщение только в Java-чат, не добавляя в дефолтный чат
	// Отправка сообщения в Java-чат
	char fullMessage[MAX_MESSAGE_LENGTH * 3 + 1];
	if (type == CHAT_TYPE_CHAT && szNick && szNick[0] != '\0') {
		sprintf(fullMessage, "{%06X}%s: {%06X}%s", dwNickColor >> 8, szNick, dwTextColor >> 8, szString);
	} else if (type == CHAT_TYPE_INFO) {
		sprintf(fullMessage, "{%06X}%s", dwTextColor >> 8, szString);
	} else if (type == CHAT_TYPE_DEBUG) {
		sprintf(fullMessage, "{%06X}%s", dwTextColor >> 8, szString);
	} else {
		sprintf(fullMessage, "{%06X}%s", dwTextColor >> 8, szString);
	}
