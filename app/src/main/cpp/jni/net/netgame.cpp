...
// Показать окно выбора серверов
void CNetGame::ShowServerSelector()
{
    if (pJava)
    {
...
// Скрыть окно выбора серверов
void CNetGame::HideServerSelector()
{
    if (pJava)
    {
        pJava->HideIngameServerSelector();
    }
}

// Отключиться от текущего сервера
void CNetGame::Disconnect()
{
    if (GetGameState() == GAMESTATE_CONNECTED || 
        GetGameState() == GAMESTATE_AWAIT_JOIN || 
        GetGameState() == GAMESTATE_CONNECTING)
    {
        ...
        // Отображение сообщения об отключении
        if (pChatWindow)
        {
            pChatWindow->AddInfoMessage("{42aaff}|{FFFFFF} Disconnected from server.");
        }
    }
}

// Подключиться к выбранному серверу
void CNetGame::ConnectToServer()
{
    ...
    // Устанавливаем состояние ожидания подключения
    SetGameState(GAMESTATE_WAIT_CONNECT);
    
    // Отображение сообщения о подключении (без IP адреса)
    if (pChatWindow)
    {
        pChatWindow->AddInfoMessage("{42aaff}|{FFFFFF} Connecting to server...");
    }
    
    // Устанавливаем время последней попытки подключения
    m_dwLastConnectAttempt = GetTickCount();
}

// Установить хост и порт сервера
void CNetGame::SetGameHost(const char* szHost, int iPort)
{
    if (szHost && strlen(szHost) < sizeof(m_szHostOrIp))
    {
        strncpy(m_szHostOrIp, szHost, sizeof(m_szHostOrIp));
        m_iPort = iPort;
    }
}
