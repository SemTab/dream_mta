...
public:
	...
	void ShowServerSelector(); // Показать окно выбора серверов
	void HideServerSelector(); // Скрыть окно выбора серверов
	void Disconnect(); // Отключиться от текущего сервера
	void ConnectToServer(); // Подключиться к выбранному серверу
	void SetGameHost(const char* szHost, int iPort); // Установить хост и порт сервера
