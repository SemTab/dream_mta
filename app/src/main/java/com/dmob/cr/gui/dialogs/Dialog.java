...
public class Dialog {
    ...
    LinearLayout pon;
    private Hud mHudManager;

    public Dialog(Activity activity) {
        aactivity = activity;
        pon = activity.findViewById(R.id.dw_root);
        this.mHudManager = new Hud(activity);
    ...
    private TextView createButtonFromOrig(TextView textView, boolean z, boolean z2) {
        if (textView == null) {
            Log.e("Dialog", "textView is null, cannot create button from it.");
            return null; // or create a default TextView
        }
                ...
                while (i6 < strArr2.length) {
                    if (z2) {
                        ...
                        for (String str6 : getSplittedTabs(strArr2[i6])) {
                            TextView buttonFromOrig = createButtonFromOrig(textView, true, false);
                            if (buttonFromOrig != null) { // Ensure the button is created
                                TextView newTextView = new TextView(mActivity);
                                newTextView.setText(Utils.transfromColors(str6));
                                newTextView.setTypeface(buttonFromOrig.getTypeface());
                                newTextView.setTextSize(13);
                                linearLayout3.addView(newTextView);
                            }
                        }
                        if(i6 == 0) {
                            ...
                            mClickedButton = 1;
                            mListitemToSend = i6;
                            mSelectedButton.setBackground(ResourcesCompat.getDrawable(Dialog.this.mActivity.getResources(), R.drawable.br_list_active, null));
                        }
                        int finalI1 = i6;
                            ...
                            public void onClick(View view) {
                                if (mSelectedButton != view) {
                                    if (mSelectedButton != null) {
                                        mSelectedButton.setBackground(ResourcesCompat.getDrawable(Dialog.this.mActivity.getResources(), R.drawable.br_list_inactive, null));
                                    }
                                    mSelectedButton = view;
                                    mClickedButton = 1;
                                    mListitemToSend = finalI1;
                                    mSelectedButton.setBackground(ResourcesCompat.getDrawable(Dialog.this.mActivity.getResources(), R.drawable.br_list_active, null));
                                    mSelectedButton.startAnimation(AnimationUtils.loadAnimation(Dialog.this.mActivity, R.anim.button_click));
                                    return;
                                }
                                mClickedButton = 1;
                                ifOtherMethodClosed = true;
