...
import com.dmob.cr.gui.IngameServerSelector;
import com.dmob.cr.gui.Numbers;
import com.dmob.cr.gui.SpawnSelector;
import com.dmob.cr.gui.antibot.CaptchaMain;
import com.dmob.cr.gui.bussines.AZS;
import com.dmob.cr.gui.chatedgar.ChatManager;
import com.dmob.cr.gui.clientsettings.DialogClientSettings;
import com.dmob.cr.gui.LoadingScreen;
import com.dmob.cr.gui.hud.Hud;
import com.dmob.cr.gui.Notification;
import com.dmob.cr.gui.Welcome;
import com.dmob.cr.gui.dialogs.Dialog;
import com.dmob.cr.gui.hud.Speedometer;
import com.dmob.cr.gui.tab.Tab;
...
public abstract class NvEventQueueActivity
    ...
    public static boolean hud_status;
    ...
    private LoadingScreen mCustomScreen = null;
    private Tab mTab = null;
    private Welcome mWelcome = null;
    private Notification mNotification = null;
    private IngameServerSelector mServerSelector = null;
    ...
    public native void sendChatMessage(byte[] messages);
    ...
    public native void sendServerSelected(String ip, int port, int x2);
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (supportPauseResume && !ranInit) {
            System.out.println("Calling init(false)");
            // Устанавливаем флаг до вызова init()
            ranInit = true;
            ...
            try {
                ...
                System.err.println("Предотвращена ошибка переполнения стека: " + e.getMessage());
            }
        }
    ...
    public void onDestroy() {
        ...
        super.onDestroy();
        systemCleanup();
        if (mCustomScreen != null) {
            mCustomScreen.onDestroy();
        }
    }
    ...
    protected boolean systemInit() {
        ...
        rpc_gps = findViewById(R.id.rpc_gps);
        rpc_greenzone = findViewById(R.id.rpc_greenzone);
        ...
        mCustomScreen = new LoadingScreen(this);
        ...
        mNumbers = new Numbers(this);
        mServerSelector = new IngameServerSelector(this);

        // Инициализация клавиатуры перед чатом
        new KeyBoard(this);
    ...
    public void callLauncherActivity(final int type) {
        if (type == 1) {
            runOnUiThread(new Runnable() {
                public void run() {
                    // rpc_gps_status = true;
                    rpc_gps.setVisibility(View.VISIBLE);
                }
            });
        }
        if (type == 2) {
        ...
        if (type == 3) {
            runOnUiThread(new Runnable() {
                public void run() {
                    ...
                    rpc_greenzone.setVisibility(View.VISIBLE);
                }
            });
        }
        if (type == 4) {
            runOnUiThread(new Runnable() {
                public void run() {
                    ...
                    rpc_greenzone.setVisibility(View.INVISIBLE);
                }
            });
        }
        ...
        if (type == 5) {
            runOnUiThread(new Runnable() {
                public void run() {
                    if (rpc_greenzone_status) rpc_gps.setVisibility(View.VISIBLE);
                    if (rpc_gps_status) rpc_gps.setVisibility(View.VISIBLE);
                    // if(hud_status) showHud();
                }
            });
        }
        if (type == 6) {
    ...
    public void showAZS(int fuelId, int maxFuel, float currentFuel, int price, int balance) {
    ...
    public void updateHudInfo(int health, int armour, int hunger, int weaponid, int ammo, int ammoinclip, int playerid, int money, int wanted, int level) {
        runOnUiThread(() -> {
            mHudManager.UpdateHudInfo(health, armour, hunger, weaponid, ammo, ammoinclip, playerid, money, wanted, level);
        });
    }

    public void updateLoading(int status) {
        runOnUiThread(() -> {
            mCustomScreen.UpdateScreen(status);
        });
    }
    ...
    public void showWelcome(boolean isRegister) {
        runOnUiThread(() -> mWelcome.show(isRegister));
    }
    ...
    public void showSpawnSelector(boolean home, boolean fraction) {
        runOnUiThread(() -> {
            try {
                if (this.mSpawnSelector != null) {
                    this.mSpawnSelector.show(home, fraction);
                } else {
                    Log.e("NvEventQueueActivity", "mSpawnSelector is null");
                }
            } catch (Exception e) {
                Log.e("NvEventQueueActivity", "Error showing spawn selector: " + e.getMessage());
            }
        });
    }
    ...
    public native boolean getCaptcha();
    ...
    public void showIngameServerSelector() {
        runOnUiThread(() -> {
            if (mServerSelector != null) {
                mServerSelector.show();
            }
        });
    }

    public void hideIngameServerSelector() {
        runOnUiThread(() -> {
            if (mServerSelector != null) {
                mServerSelector.hide();
            }
        });
    }
    ...
    public void showChat() {
        runOnUiThread(() -> {
            if (mChatManager != null) {
                ...
                Utils.ShowLayout(mChatManager.chat, true);
                mChatManager.ChatOpen();
            }
        });
    }
    ...
    public void addChatMessage(String message) {
        runOnUiThread(() -> {
            if (mChatManager != null) {
                // Исходное сообщение передаётся напрямую в ChatManager, 
                // где уже происходит обработка кодировки
                mChatManager.AddChatMessage(message);
            }
        });
    }

    /**
     * Метод для добавления сообщения с указанием исходной кодировки
     * Полезно, когда известно, что сообщение приходит в конкретной кодировке
     * 
     * @param messageBytes массив байтов сообщения
     * @param encoding кодировка сообщения (например, "windows-1251", "UTF-8")
     */
    public void addChatMessageWithEncoding(byte[] messageBytes, String encoding) {
        try {
            // Преобразуем байты в строку с указанной кодировкой
            String message = new String(messageBytes, encoding);
            ...
            e.printStackTrace();
            // В случае ошибки добавляем сообщение как есть, используя UTF-8
            addChatMessage(new String(messageBytes));
        }
    }
    ...
    public void setChatStatus(int status) {
        runOnUiThread(() -> {
            if (mChatManager != null) {
                mChatManager.setChatStatys(status);
            }
        });
    }
    ...
    // Вспомогательный метод для отправки сообщения в формате UTF-8
    public void sendChatMessageUTF8(String message) {
        try {
            sendChatMessage(message.getBytes("UTF-8"));
        } catch (UnsupportedEncodingException e) {
            e.printStackTrace();
        }
    }
    ...
    public void showKeyboard() {
        runOnUiThread(() -> {
            if (KeyBoard.getKeyBoard() != null) {
                ...
                // Если клавиатура не инициализирована, создаем новый экземпляр
                new KeyBoard(this);
                if (KeyBoard.getKeyBoard() != null) {
                    KeyBoard.getKeyBoard().OpenKeyBoard(null);
                }
    ...
    public native void sendAuthData(String login, String password);
    
    public native void sendRegisterData(String login, String password, String email, int nation, int age);
    ...
    public static void fixEditTextForAndroid10Xiaomi(EditText editText) {
        if (editText != null && Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q && 
